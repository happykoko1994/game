...
io.on('connection', (socket) => {
    ...
    socket.on('submitAnswer', (answer) => {
        ...
        if (player && !player.answered) {
            ...
            const matchedAnswer = question.answers.find((a) => a.text.toLowerCase() === answer.toLowerCase());
            if (matchedAnswer) {
                player.answered = true;
                io.emit('log', `${player.name} guessed: ${matchedAnswer.text} (${matchedAnswer.points} points)`);
